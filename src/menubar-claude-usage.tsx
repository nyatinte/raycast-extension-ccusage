...
export default function MenuBarClaudeUsage() {
  ...
  if (stats.error) {
    return (
      <MenuBarExtra 
        icon={{ source: Icon.ExclamationMark, tintColor: Color.Red }} 
        tooltip="Error loading usage data"
      >
        <MenuBarExtra.Item
          title="Error"
          subtitle={stats.error}
          icon={Icon.ExclamationMark}
        />
      </MenuBarExtra>
    );
          ...
          <>
            ...
            {stats.recentSessions.slice(0, 3).map((session, index) => (
              <MenuBarExtra.Item
                ...
                icon={session.model.includes("opus") ? Icon.Crown : 
                      session.model.includes("sonnet") ? Icon.Sparkles : 
                      session.model.includes("haiku") ? Icon.Leaf : Icon.Message}
              />
            ))}
          </>
        ) : (
