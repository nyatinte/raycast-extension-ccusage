...
export class UsageCalculator {
  static calculateDailyGrowth(current: DailyUsageData | null, previous: DailyUsageData | null): {
    tokenGrowth: number;
    costGrowth: number;
    ...
    const tokenGrowthPercentage = previous.totalTokens > 0 
      ? `${((tokenGrowth / previous.totalTokens) * 100).toFixed(1)}%`
      : "0%";
    
    const costGrowthPercentage = previous.cost > 0
      ? `${((costGrowth / previous.cost) * 100).toFixed(1)}%`
      : "0%";
  ...
  static estimateRemainingBudget(currentCost: number, dailyBudget: number, daysInMonth: number): {
